...
class MinimumVariance:
    ...
    def calculate_volatility(self, data):
        """
        Calcule la volatilité historique des rendements journaliers pour un actif.
        """
        if len(data) < 2:
            return None
        data['Rendements'] = np.log(data['Adj Close'] / data['Adj Close'].shift(1))
        return data['Rendements'].std() * np.sqrt(252)  # Annualiser la volatilité
    ...
    def select_top_assets(self, current_date):
        ...
        for ticker in self.tickers:
            ...
            if volatility is not None:
                ...
                # print(f"Volatilité pour {ticker} : {volatility}")

        # Trier par volatilité croissante (les plus faibles d'abord)
        sorted_assets = sorted(volatility_scores.items(), key=lambda x: x[1])
