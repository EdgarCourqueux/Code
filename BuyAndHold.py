...
class BuyAndHold(Indicateurs):
    def __init__(self, montant_initial, date_investissement, tickers):
    ...
    def execute(self):
        ...
        for ticker in self.tickers:
            data = yf.download(ticker, start="2000-01-01", end=self.date_du_jour.strftime('%Y-%m-%d'))
            if data.empty:
            ...
            # Validation de la date d'investissement pour chaque ticker
            try:
                date_investissement_proche = data.loc[data['Date'] >= self.date_investissement, 'Date'].iloc[0]
            except IndexError:
                print(f"La date d'investissement est hors de la plage des données disponibles pour {ticker}.")
                continue

            date_du_jour_proche = data.loc[data['Date'] <= self.date_du_jour, 'Date'].iloc[-1]
